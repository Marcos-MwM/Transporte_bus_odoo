...
class TransportePublico(models.Model):
    ...
    imagen_de_referencia= fields.Image(string='Imagen de referencia',max_width=800,
    max_height=800)
    ...
    bus_identificacion = fields.Char(string='ID único del bus', required=True ,default='ID automatico',copy=False)
    ...
    def create(self, vals):
        if vals.get('bus_identificacion', 'ID automatico') == 'ID automatico':
            vals['bus_identificacion'] = self.env['ir.sequence'].next_by_code(
                'transporte.ruta'
            ) or 'ID automatico'
